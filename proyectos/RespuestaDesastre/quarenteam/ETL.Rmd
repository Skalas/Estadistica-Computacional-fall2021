...
data_raw <- path %>% 
  ...
  #filter(!(is.na(`COORDENADAS GEOGRAFICAS`) | is.na(...9) | is.na(...10))) %>% 
  #filter(!(`COORDENADAS GEOGRAFICAS` == 'N')) #%>% 
  #filter(!(`COORDENADAS GEOGRAFICAS` == 'LATITUD'))#| ...9 == 'w' | ...10 == 'MSNM')))
...
glimpse(refugios %>% filter(USO_DEL_INMUEBLE == 'RELIGIOSOS'))
glimpse(refugios %>% filter(USO_DEL_INMUEBLE  == 'GOBIERNO MUNICIPAL'))
glimpse(refugios %>% filter(USO_DEL_INMUEBLE  == 'MUNICIPAL'))
